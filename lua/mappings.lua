...
map("i", "<C-q>", "=")
map("i", "<D-'>", "=")
map("v", "F", "yo<Esc>oif (!<Esc>pa)<CR>return NULL;<Esc>")
map("i", "<C-->", "_")
map("n", "dW", "ciW")
map("n", "D", "ciW")
map("n", "L", "A")
...
-- map("n", "<C-;>", "gd", { noremap = true, silent = true })
...
-- vim.keymap.set("n", "Y", [["+Y]], { desc = "Copy line to clipboard" })
map("n", "<leader>s", ":HopWord<CR>")
map("n", "<leader>y", ":HopYankChar1<CR>")
